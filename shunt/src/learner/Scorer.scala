...
import qd.FValue.ValueHasOneAndZero

class Scorer(edb: Config[FValue], refOut: Config[FValue]) {
  val outputRels: Set[Relation] = refOut.keySet
  val allTuples: Map[Relation, Set[DTuple]] = outputRels.map(rel => rel -> rel.populate).toMap

  def gradient(pos: Map[Token, Double], out: Config[FValue], rel: Relation, t: DTuple): TokenVec = {
  ...
  def rmse(out: Config[FValue]): Double = {
  ...
  def cutoffL2(p: Program[FValue], cutoff: Double): (Program[FValue], Double, Set[Token]) = {
    ...
    val highRules = p.rules.filter(_.coeff.toDouble >= cutoff).map(r =>
      Rule(r.name, FValue(1.0, r.coeff.prov), r.head, r.body)
    )
    val pHi = Program(p.name, highRules)
    val out = SeminaiveEvaluator(pHi)(ValueHasOneAndZero)(edb)
  ...
  def s0(out: Config[FValue]): Double = {
  ...
  def gradS0(pos: TokenVec, out: Config[FValue]): TokenVec = {
  ...
  def s1(out: Config[FValue]): Double = {
  ...
  def gradS1(pos: TokenVec, out: Config[FValue]): TokenVec = {
