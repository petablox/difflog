...
class CallSite1Scalability extends FunSuite {
  val name: String = "CallSite1"
  ...
  val refOut : Config[FValue] = Config[FValue] (
  ...
  val expected: Set[Any] = Set(1, 8, 61, 93)
  val maxVarCount: Int = 6

  test(s"Scalability") {
    for (nrules <- Range(4, 95)) {
      ...
      val p = Program(s"AndersenfScalability", soup)
