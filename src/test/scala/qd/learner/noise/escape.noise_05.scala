...
@Ignore
class Genescape_5 extends Problem {
	...
	val rHHTuples = Set((0, 1),(1, 2),(2, 3),(0, 2),(1, 3),(0, 3)).map { case (x0,x1) => DTuple(Atom(x0),Atom(x1)) }
	val VHTuples = Set((1, 1),(0, 0),(2, 2),(3, 3)).map { case (x0,x1) => DTuple(Atom(x0),Atom(x1)) }
	val rRHTuples = Set((0, 1),(0, 3),(2, 1),(2, 2),(2, 3),(1, 0),(2, 4)).map { case (x0,x1) => DTuple(Atom(x0),Atom(x1)) }
