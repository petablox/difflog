...
object NaiveEvaluator extends Evaluator {
  ...
  override def apply[T <: Value[T]](rules: Set[Rule[T]], edb: Config[T])(implicit vs: Semiring[T]): Config[T] = {
    ...
    while (state.changed) { state = immediateConsequence(state) }
    state.config
  }
  ...
  case class State[T <: Value[T]](rules: Set[Rule[T]], config: Config[T], changed: Boolean)
    ...
    def resetChanged: State[T] = if (!changed) this else State(rules, config, changed = false)
    def setChanged: State[T] = if (changed) this else State(rules, config, changed = true)

  }

  def immediateConsequence[T <: Value[T]](state: State[T]): State[T] = {
    state.rules.foldLeft(state.resetChanged)((nextState, rule) => ruleConsequence(nextState, rule))
  }
  ...
  def ruleConsequence[T <: Value[T]](state: State[T], rule: Rule[T]): State[T] = {
    ...
    for (literal <- rule.body) {
      assignments = extendAssignments(literal, state, assignments)
  ...
  def extendAssignments[T <: Value[T]](
    ...
    for (assignment <- assignments;
         ...
         (tuple, score) <- state.config(literal.relation).filter(f);
